...
export type DashboardChartFilters = {
  ...
  /** Bumped after dashboard finishes loading filter metadata. */
  chartsRevision?: Ref<number>
...
export function useDashboardCharts(filters: DashboardChartFilters) {
  ...
  const chartsLoadKey = computed(
    () => `${dateKey.value}::${filters.chartsRevision?.value ?? 0}`,
  )
  ...
  if (filters.chartsRevision) {
    watchDebounced(chartsLoadKey, () => void refresh(), { debounce: 150, maxWait: 600, immediate: true })
  } else {
    watchDebounced(filterKey, () => void refresh(), { debounce: 150, maxWait: 600, immediate: true })
  }
