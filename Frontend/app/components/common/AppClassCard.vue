...
import { formatClassDuration, parseDurationMonths } from "~/utils/format/duration";
...
const classDurationRaw = computed(() =>
  pickFirstText([
    props.classItem.classDuration,
    props.classItem.duration,
    props.classItem.durationClass,
    props.classItem.courseDuration,
  ]),
);

const classDurationLabel = computed(() => {
  const raw = classDurationRaw.value
  if (!raw) return ''
