...
type RangeModel = { start?: unknown; end?: unknown }
...
const emit = defineEmits<{
  (e: 'update:range', val: RangeModel): void
}>()
...
<template>
  <UPopover :content="{ align: 'end' }">
    ...
    <UButton color="neutral" variant="subtle" icon="i-lucide-calendar" trailing-icon="i-lucide-chevron-down"
    ...
    <template #content>
      <div class="flex flex-col bg-background rounded-lg overflow-hidden min-w-[200px]">
        ...
        <UCalendar v-model="dateRange" class="p-2" :number-of-months="1" range />
        <!-- Sidebar panel -->
        <div class="p-4 bg-muted/20 border-t flex flex-col gap-2 justify-end">
          <UButton :label="t('components.reset')" size="xs" variant="ghost" color="neutral" icon="i-lucide-rotate-ccw"
            @click="resetRange" />
        </div>
      </div>
    </template>
