...
async function drawCertificateProfile(ctx: CanvasRenderingContext2D, data: ReturnType<typeof certificateDetails>) {
  const src = certificateProfileImage.value || props.studentImage?.trim()
  if (!src) return
...
const footerSums = computed(() => {
  ...
  return {
    ...
    discountPrice: sumNum((r) => Number(r.discountPrice || 0)),
    priceAfterDiscount: sumNum((r) => Number(r.priceAfterDiscount || 0)),
  }
})
      ...
      <div class="flex min-h-0 flex-1 flex-col gap-3">
        ...
        >
          <div class="flex min-h-0 flex-col items-center justify-center">
            <div v-if="isGeneratingCertificate" class="py-8 text-sm text-muted-foreground">
              {{ $t('common.loading') }}
            </div>
            <canvas
        ...
        >
          ...
          <template #discountPrice-cell="{ row }">
            <span class="tabular-nums">{{ formatCurrency(Number(row.original.discountPrice || 0), 'USD') }}</span>
          </template>
          <template #priceAfterDiscount-cell="{ row }">
            <span class="tabular-nums font-medium text-foreground">{{
              formatCurrency(Number(row.original.priceAfterDiscount || 0), 'USD')
            }}</span>
          </template>
        </TableApptable>
      </div>
