...
const chartsRenderKey = computed(
  () =>
    `${selectedCategories.value.map((i) => i.value).join(',')}|${selectedCourses.value.map((i) => i.value).join(',')}|${selectedClasses.value.map((i) => i.value).join(',')}|${chartsRevision.value}`,
)
        ...
        >
          ...
          <UCard class="shadow-sm border-accented relative min-h-0 overflow-hidden flex flex-col flex-1">
            ...
            <div class="w-full relative flex-1 min-h-0 p-2">
              <div
                v-if="chartsLoading"
                class="absolute inset-0 flex items-end gap-2 p-4"
              >
